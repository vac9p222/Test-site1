	...
	function bootstrap_theme_entry_footer() {
		...
		if ( 'post' === get_post_type() ) {
			...
			if ( $categories_list ) {
				...
				printf( '<span class="cat-links">' . esc_html__( 'Posted in %1$s', 'bootstrap-theme' ) . '</span>', $categories_list ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
			}
		...
		edit_post_link(
			sprintf(
				wp_kses(
					...
					__( 'Edit <span class="screen-reader-text">%s</span>', 'bootstrap-theme' ),
					array(
						'span' => array(
							'class' => array(),
						),
...
if ( ! function_exists( 'bootstrap_theme_post_thumbnail' ) ) :
	/**
	 ...
	 */
	function bootstrap_theme_post_thumbnail() {
